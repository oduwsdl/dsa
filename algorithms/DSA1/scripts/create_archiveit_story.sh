...
stop_mementodamage () {
    docker stop memento-damage
    docker rm memento-damage
}

restart_mementodamage () {
    stop_mementodamage
    docker run -d -p 32768:80 --name memento-damage erikaris/memento-damage:latest /app/entrypoint.sh
}
...
if [ ! -e ${metadata_report} ]; then
    echo "`date` --- executing command"
    hc report metadata -i archiveit -a ${collection_id} -cs ${cache_storage} -o ${metadata_report} -l ${working_directory}/${collection_id}-report-metadata-`date '+%Y%m%d%H%M%S'`.log  -e ${errorfile}
else
...
if [ ! -e ${entity_report} ]; then
    echo "`date` --- executing command"
    hc report entities -i mementos -a ${mementos_file} -cs ${cache_storage} -o ${entity_report} -l ${working_directory}/${collection_id}-report-entities-`date '+%Y%m%d%H%M%S'`.log  -e ${errorfile}
else
...
if [ ! -e ${sumgram_report} ]; then
    echo "`date` --- executing command"
    hc report terms -i mementos -a ${mementos_file} -cs ${cache_storage} -o ${sumgram_report} --sumgrams -l ${working_directory}/${collection_id}-report-terms-`date '+%Y%m%d%H%M%S'`.log  -e ${errorfile}
else
...
if [ ! -e ${image_report} ]; then
    echo "`date` --- executing command"
    hc report image-data -i mementos -a ${mementos_file} -cs ${cache_storage} -o ${image_report} -l ${working_directory}/${collection_id}-report-imagedata-`date '+%Y%m%d%H%M%S'`.log  -e ${errorfile}
else
...
if [ ! -e ${story_data_file} ]; then
    echo "`date` --- executing command:::"
    hc synthesize raintale-story -i mementos -a ${mementos_file} -o ${story_data_file} -cs ${cache_storage} --imagedata ${image_report} --title "Archive-It Collection" --termdata ${sumgram_report} --entitydata ${entity_report} --collection_metadata ${metadata_report}  -e ${errorfile}
else
